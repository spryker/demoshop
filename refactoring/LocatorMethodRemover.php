...
class LocatorMethodRemover extends AbstractRefactorer
{
    ...
    {
        $searchPattern = '/\/\*\*\n\s+\*\s+@return\s(?:.*?)\n+\s+\*\/\n\s+protected\sfunction\sgetLocator\(\)\n\s+{\n\s+(?:.*);\n\s+\}\n+/';

        $content = $this->file->getContents();

        if (!preg_match('/private \$locator;/', $content) && preg_match($searchPattern, $content, $matches)) {
