...
SELECT
  ...
  FROM (
    SELECT * FROM spy_category_node n
    INNER JOIN spy_category c ON c.id_category = n.fk_category AND c.is_active = TRUE AND c.is_in_menu = TRUE
    ) spy_category_node_rec,
    (SELECT @pv := root.id_category_node 
      FROM 
        spy_category_node root
        INNER JOIN spy_category c ON c.id_category = root.fk_category AND c.is_active = TRUE AND c.is_in_menu = TRUE
        INNER JOIN spy_touch t ON (
        root.id_category_node = t.item_id
          AND t.item_event = :spy_touch_item_event
          AND t.touched >= :spy_touch_touched
          AND t.item_type = :spy_touch_item_type
        )
      WHERE 
      root.is_root = TRUE) initialisation
