    ...
    protected function createProductImages(array $sortedImageFiles)
    {
        ...
        foreach ($this->facadeImage->getImageSizes() as $size) {
            foreach ($sortedImageFiles as $file) {
                ...
                $imageEntityId = $this->facadeImage->addImage($originalImageFilename, '/tmp', $mappingId, $size->getIdImageSize(), $order, '-' . $size->getName(), $extension, \ProjectA\Zed\Image\Persistence\Propel\Map\PacImageTableMap::COL_COL_TYPE_PRODUCT, array());
