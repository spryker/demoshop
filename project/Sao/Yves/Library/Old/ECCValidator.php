...
/**
 *
 * Sao_Yves_Library_ECCValidator class
 *
...
class Sao_Yves_Library_ECCValidator extends CValidator
{
    ...
    const VALIDATION_ATTRIBUTE_NUMBER = 'number';
    const VALIDATION_ATTRIBUTE_CARDHOLDER = 'cardholder';
    const VALIDATION_ATTRIBUTE_DATE = 'date';
     ...
     * @var string
     */
    public $validationAttribute = self::VALIDATION_ATTRIBUTE_NUMBER;
    ...
    protected function validateAttribute($object, $attribute)
    {
        ...
        $method = 'validate' . ucfirst($this->validationAttribute);
        if (method_exists($this, $method) && !$this->$method($value)) {
            $message = $this->message !== null
                ? $this->message
                : ProjectA_Yves_Library_Yii::t(
                    'ECCValidator', '{attribute} is not a valid Credit Card number.'
                );
    ...
    public function validateNumber($creditCardNumber)
    {
        ...
        if (!$this->checkType()) {
            throw new CException(ProjectA_Yves_Library_Yii::t(
                'ECCValidator', 'The "format" property must be specified with a supported Credit Card format.'
            ));
        }
     ...
     * @return bool
     */
    public function validateCardholder($creditCardHolder)
    {
    ...
    public function validateAll($creditCardHolder, $creditCardNumber, $creditCardExpiredMonth, $creditCardExpiredYear)
    {
        ...
        return $this->validateName($creditCardHolder) && $this->validateNumber($creditCardNumber)
